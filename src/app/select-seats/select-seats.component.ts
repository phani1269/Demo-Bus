...
import { IBoardingPoints, IbookSeat, Ibus,  IbusId,  IDroppingPoints,  ISeat } from '../models/bus';
...
BusBoardingPoints(Object5: any,Object6:any,Object4:any) {
  this._dataService.getRoutePointsWebApi("api/BusBooking/GetRoutePointsbyID",Object5,Object6,Object4).subscribe(
    {
      next: (out) => {
...
this.sum-=c[0];
this.total=this.total-c[0]-this.bus.busPrice;
 
 
}

BusData() {
  this._dataService.getBusById("api/BusBooking/GetBusDetailsbyIDs",this.busID).subscribe(
    {
      next: (out) => {
...
localStorage.setItem("bus",JSON.stringify(this.bus));
  if(ischecked){
    ...
    this.fill= this.seats.find((x:{id:any})=>x.id==a);
      ...
      this.total=(this.fillseat.length*this.bus.busPrice)+this.sum;
...
buttonClick(){
  ...
  const b = this.bus.busPrice;
    this.fillseat.forEach((element: any) => {
        element.Price=element.Price+this.bus.busPrice;
    });
