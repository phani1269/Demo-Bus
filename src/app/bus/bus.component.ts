...
import { Ibus, IDestination, IOrigin } from '../models/bus';
...
export class BUSComponent implements OnInit {
 ...
 busName:any
  ...
  ngOnInit(): void {
    ...
    this.BusDestination();
    this.PastDate();
   // this.GetAVilableBuses();
  }

  BusOrigin() {
    this._dataservice.getDataFromWebApi("api/BusBooking/GetAllOrigins").subscribe(
      {
        next: (out) => {
  ...
  BusDestination() {
    this._dataservice.getDataFromWebApi("api/BusBooking/GetAllDestinations").subscribe(
      {
        next: (out) => {
   ...
   GetAVilableBuses(data: NgForm,Object: any,Object1:any){
    this._dataservice.getBusDetailswebApi("api/BusBooking/GetAvailableBusDetails", Object,Object1).subscribe(
      {
        next: (out) => {
  ...
  ShowPrice(key: string){
    ...
    Search(){
      ...
      FilterByAC(Object: any,Object1:any) {
        this._dataservice.getBusDetailswebApi("api/BusBooking/FilterByBusTypeAC", Object,Object1).subscribe(
          {
            next: (out) => {
              this.buses = out as Ibus[];
              console.log(this.buses);
      ...
      FilterBynonAC(Object: any,Object1:any) {
        this._dataservice.getBusDetailswebApi("api/BusBooking/FilterByBusTypeNONAC", Object,Object1).subscribe(
          {
            next: (out) => {
              this.buses = out as Ibus[];
              console.log(this.buses);
